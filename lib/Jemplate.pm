...
Where "--runtime" and "runtime-opt" can include:
    ...
    --amd               Wrap the runtime in an AMD define(), suitable for requirejs, etc.
...
sub main {
    ...
    if ($runtime) {
        print runtime_source_code(@$jemplate_options{qw/runtime ajax json xhr xxx compact amd/});
...
sub get_options {
    ...
    my $source  = 0;
    my $exclude = 0;
    my ($ajax, $json, $xxx, $amd, $xhr, $compact, $minify);
    ...
    return (
        ...
        { compile => $compile, runtime => $runtime, list => $list,
            source => $source,
            exclude => $exclude,
            ajax => $ajax, json => $json, xxx => $xxx, amd => $amd, xhr => $xhr,
...
sub runtime_source_code {
    ...
    my ($runtime, $ajax, $json, $xhr, $xxx, $compact, $amd)
    	= map { defined $_ ? lc $_ : "" } @_[0 .. 6];
    ...
    push @runtime, "define('jemplate', [], function() {\n" if $amd;
